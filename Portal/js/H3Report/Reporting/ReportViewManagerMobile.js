...
FilterManager.prototype = {
                ...
                case _DefaultOptions.FilterType.Organization:
                    {
                        $("[id='" + filter.ColumnCode + "']").bind("change", function (e) {
                            var datas = [];
                            var dastr = $(this).find('input[type="hidden"]').val();
                            var da = [];
                            if (dastr != "") {
                                da = dastr.split(',');
                            }
    ...
    GetControl: function (filter) {
        ...
        if (filter.FilterType == _DefaultOptions.FilterType.String) {// 文本类型 
            ...
            var $input = '<div class="sheetuserflag menu-bar"  id="' + filter.ColumnCode + '" ng-click="ReportstateUser($event)" ng-model="filters" ' + dataOrgUnitVisible + ' ' + dataUserVisible + '  data-scopedata="' + filter.ColumnCode + '" data-datafield="' + filter.ColumnCode + '">' +
            '<div class="menu-bar-left f15">{{languages.Filter.originator}}</div>' +
                '<div class="menu-bar-right f14">' +
               '{{filters[\'' + filter.ColumnCode + '\'].Name}}' +
                 '<span ng-repeat="d in filters[\'' + filter.ColumnCode + '\']" ng-if="$index<1">' +
                 ' <i>{{d.Name}}</i>' +
                 '</span>' +
                 '<span ng-if="filters[\'' + filter.ColumnCode + '\'].length>1" ng-show="{{!languages.isShow}}">' +
                 '    等{{filters[\'' + filter.ColumnCode + '\'].length}}人' +
                 '</span>' +
                 '<span ng-if="filter[\'' + filter.ColumnCode + '\'].length>1" ng-show="{{languages.isShow}}">' +
                     '{{filters[\'' + filter.ColumnCode + '\'].length}}{{languages.personTotal}}' +
            '</span>' +
        ...
        if (this.RowTable != null && !$.isEmptyObject(this.RowTable)) {
            for (var RowI = 0; RowI < this.RowTable.length; RowI++) {
                ...
                for (var RowJ = 0; RowJ < this.RowTable[RowI].length; RowJ++) {
                    ...
                    if (that.Widget.LinkageReports == null || that.Widget.LinkageReports.length == 0) {
                        $Th = $('<th  style="width:' + MyRowTd.ColSpan * 67 + 'px"  data-columncode="' + MyRowTd.ColumnCode + '" data-code="' + MyRowTd.Code + '" data-value="' + MyRowTd.Value + '" rowspan="' + MyRowTd.RowSpan + '" colspan="' + MyRowTd.ColSpan + '">').html(MyRowTd.Value == "" ? "--" : MyRowTd.Value);
                    }
                    else {
                        $Th = $('<th  style="width:' + MyRowTd.ColSpan * 67 + 'px"  class="unittd" data-code="' + MyRowTd.Code + '" data-columncode="' + MyRowTd.ColumnCode + '" data-value="' + MyRowTd.Value + '" rowspan="' + MyRowTd.RowSpan + '" colspan="' + MyRowTd.ColSpan + '">').html(MyRowTd.Value == "" ? "--" : MyRowTd.Value);
                        $Th.unbind("click").bind("click", function () {
                            //联动,作为查询条件；
                            that.FilterData = {};
                            var UnitWidget = that.Widget;
