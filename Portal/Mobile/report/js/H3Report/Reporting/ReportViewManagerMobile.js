...
var CommonFunction = {
    ...
    PostJsonp: function (url, data, callback, async) {
        $.ajax({
            type: "POST",
            url: url,
            async: async == undefined ? true : async,
    ...
    ShowNoneItemImg: function ($container) {
        ...
        var $img = $("<img src='img/NoneReport.png' />").css("margin-top", "3%");//钉钉，微信，app公用y一个
...
ReportViewManager.prototype = {
    Init: function (ionic) {
        ...
        //独立跨域获取报表
        if (window.cordova) {
            url = ionic.$scope.setting.appServiceUrl + '/LoadReportPage';
            url += '?mobileToken=' + ionic.$scope.user.MobileToken;
            url += '&userCode=' + ionic.$scope.user.Code;
            url += '&ReportCode=' + this.Code;
            CommonFunction.GetJsonp(url, callbackFunc, this.$filterContainer ? true : false);
        } else {
            CommonFunction.Post(CommonFunction.LoadReportPage,
                { Code: this.Code },
                callbackFunc, this.$filterContainer ? true : false);
        }
    },
    // 校验
            ...
            for (var k = 0; k < this.Filters.length; k++) {
                ...
                if (Keys != "") {
                   ...
                   // var $colTitle = $("<div class=\"menu-bar-left f15\">" + this.Filters[k].DisplayName + "</div><div class=\"menu-bar-right\"></div>");
                    //  $row.append($colTitle);
                    //系统保留字段变成中文
                    if (this.Ionic.$scope.LanJson.ConditionColumns.hasOwnProperty(this.Filters[k].DisplayName)) {
                        var $colTitle = $("<div class=\"menu-bar-left f15\">" + this.Ionic.$scope.LanJson.ConditionColumns[this.Filters[k].DisplayName] + "</div><div class=\"menu-bar-right\"></div>");
                        this.Filters[k].DisplayName = this.Ionic.$scope.LanJson.ConditionColumns[this.Filters[k].DisplayName];
                    } else {
                        var $colTitle = $("<div class=\"menu-bar-left f15\">" + this.Filters[k].DisplayName + "</div><div class=\"menu-bar-right\"></div>");
                    }
                    $row.append($colTitle);
    ...
    Init: function () {
        ...
        if (window.cordova) {
            var postData = {
                ...
                ObjectId: that.Widget.ObjectID,
                Code: that.ReportPage.Code,
                UnitFilterDataJson: JSON.stringify(that.UnitFilterDataJson)
            };
            postData.mobileToken = that.ReportViewManager.Ionic.$scope.user.MobileToken;
            postData.userCode = that.ReportViewManager.Ionic.$scope.user.Code;
            url = that.ReportViewManager.Ionic.$scope.setting.appServiceUrl + '/LoadChartsData';
            if (postData.UnitFilterDataJson == undefined) {
                postData.UnitFilterDataJson = "";
            }
            if (postData.FilterData == undefined) {
                postData.FilterData = "";
            }
            CommonFunction.PostJsonp(url, postData, callbackFunc);
        } else {
            //加载报表数据源数据
            CommonFunction.Post(
...
GridViewManager.prototype = {
    Init: function () {
        ...
        var callbackFunc = function (data) {
            ...
            //兼容独立APP跨域请求
            var dataType = "json";
            var dataUrl = CommonFunction.ActionUrl + "/" + CommonFunction.LoadGridData + "?Command=" + CommonFunction.LoadGridData + "&WidgetID=" + that.Widget.ObjectID + "&Code=" + that.ReportPage.Code;
            if (window.cordova) {
                dataUrl = that.ReportViewManager.Ionic.$scope.setting.appServiceUrl + '/LoadGridData';
                dataType = "jsonp";
            }
            that.mydatatable = $Table.DataTable({
                ...
                "ajax": {
                    ...
                    data: function (d) {
                        ...
                        if (param["UnitFilterDataJson"] == undefined) {
                            param["UnitFilterDataJson"] = "";
                        }
                        if (param["FilterData"] == undefined) {
                            param["FilterData"] = "";
                        }
                        ...
                        if (window.cordova) {
                            param["WidgetID"] = that.Widget.ObjectID;
                            param["Code"] = that.ReportPage.Code;
                            param["mobileToken"] = that.ReportViewManager.Ionic.$scope.user.MobileToken;
                            param["userCode"] = that.ReportViewManager.Ionic.$scope.user.Code;
                        }
                        return param;
                    },
                },
        ...
        //兼容独立APP跨域请求
        if (window.cordova) {
            var postData = { FilterData: JSON.stringify(that.FilterData), WidgetID: that.Widget.ObjectID, Code: that.ReportPage && that.ReportPage.Code ? that.ReportPage.Code : that.ReportPage.Code, UnitFilterDataJson: JSON.stringify(that.UnitFilterDataJson), "start": 0, "length": 0, "orderby": "", "dir": "" };
            postData.mobileToken = that.ReportViewManager.Ionic.$scope.user.MobileToken;
            postData.userCode = that.ReportViewManager.Ionic.$scope.user.Code;
            url = that.ReportViewManager.Ionic.$scope.setting.appServiceUrl + '/LoadGridData';
            if (postData.UnitFilterDataJson == undefined) {
                postData.UnitFilterDataJson = "";
            }
            if (postData.FilterData == undefined) {
            ...
            CommonFunction.Post(
               ...
               { FilterData: JSON.stringify(that.FilterData), WidgetID: that.Widget.ObjectID, Code: that.ReportPage && that.ReportPage.Code ? that.ReportPage.Code : that.ReportPage.Code, UnitFilterDataJson: JSON.stringify(that.UnitFilterDataJson), "start": 0, "length": 0 },
               callbackFunc);
        }
    },
...
SimpleBoardChildManager.prototype = {
    Init: function () {
        ...
        if (window.cordova) {
            var postData = {
                FilterData: JSON.stringify(that.FilterData),
                WidgetObjectId: that.Widget ? that.Widget.ObjectID : "",
                ReportPageObjectId: that.reportPage ? that.reportPage.Code : "",
                ReportWidgetSimpleBoardObjectId: that.WidgetSimpleBoard ? that.WidgetSimpleBoard.ObjectID : null,
                UnitFilterDataJson: JSON.stringify(that.UnitFilterDataJson),
                //WidgetSimpleBoard: JSON.stringify(that.Widget)
                WidgetSimpleBoard: ""//该参数不传 因为该数据比较多 跨域jsonp 会导致url过长报错
            };
            postData.mobileToken = that.ReportViewManager.Ionic.$scope.user.MobileToken;
            postData.userCode = that.ReportViewManager.Ionic.$scope.user.Code;
            url = that.ReportViewManager.Ionic.$scope.setting.appServiceUrl + '/LoadSimpleBoard';
            if (postData.UnitFilterDataJson == undefined) {
                postData.UnitFilterDataJson = "";
            }
            if (postData.FilterData == undefined) {
            ...
            if (postData.WidgetSimpleBoard == undefined) {
                postData.WidgetSimpleBoard = "";
            }
            CommonFunction.PostJsonp(url, postData, callbackFunc);
        } else {
            //加载报表数据源数据
