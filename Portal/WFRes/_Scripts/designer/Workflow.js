...
Workflow.prototype = {
    ...
    initActions: function () {
        ...
        //.bind("click" + WorkflowSettings.EventNameSpace, this.doClick);
        $(document).unbind("click.unbindInstaneTableLock").bind("click.unbindInstaneTableLock", function (e) {
            e = e || window.event;
            InstaneTableLock = true;
        });
    ...
    doDblClick: function (e) {
        ...
        if (_FocusActivity && WorkflowMode.ViewOnly) {
            if ((typeof (WorkflowHandlerUrl)) != "undefined" && $.inArray(WorkflowMouseMoveStack.FocusActivity.ToolTipText, ["FillSheet", "Approve", "Circulate", "SubInstance"]) > -1) {
                var showAdjustActivityModal = $("#showAdjustActivityModal");
                if (showAdjustActivityModal && showAdjustActivityModal.parent("div").is(":visible")) {
                    showAdjustActivityModal.click();
                }
            }
        }
    },
    ...
    doMouseMove: function (e) {
        ...
        if (_FocusActivity)
            cursorCought = true;
        //点击流程节点时锁定悬浮窗
        $("." + ActivityStyleClassName.Activity).unbind("click.InstaneTableLock").bind("click.InstaneTableLock", function (e) {
            InstaneTableLock = false;
            e.stopPropagation();
        })
        ...
        if (workflowMode == WorkflowMode.Designer) {
            ...
            if (_FocusActivity) {
                ...
                WorkflowMouseMoveStack.FocusActivity = _FocusActivity;
                //$scope.Selected.ActivityCode = _FocusActivity.ActivityCode;
                ...
                if (($(WorkflowMouseMoveStack.FocusActivity.htmlObject).hasClass("ActivityWorking") || $(WorkflowMouseMoveStack.FocusActivity.htmlObject).hasClass("ActivityFinished")) ||
                    ...
                    if (typeof (ActivityInstaneDiv) == "undefined") {
                        ...
                        //ActivityInstaneDiv.append($("<div><span style='padding-left:15px'>流程编码：" + _FocusActivity.ActivityCode + "</span></div>"));
                    }
                    ...
                    if ($(WorkflowMouseMoveStack.FocusActivity.htmlObject).hasClass("ActivityWorking") || $(WorkflowMouseMoveStack.FocusActivity.htmlObject).hasClass("ActivityFinished")) {
                        ...
                        ActivityTokenGrid.find("tr").each(function (index) {
                            ...
                            if (index != 0) {
                                ...
                                tds.eq(0).hide();
                                tds.eq(1).hide();
                                tds.eq(2).css("width", "60px");
                                tds.eq(3).css("width", "60px");
                                tds.eq(4).css("width", "70px");
                                tds.eq(7).css("width", "100px");
                            }
                        });
                        ActivityInstaneDiv.children("table").show();
                        ActivityInstaneDiv.children("div").show();
                        //标题不隐藏
                        ActivityInstaneDiv.children("table").children("thead").find("tr").eq(1).show();
                        //激活状态才可以调整处理人
                        ActivityInstaneDiv.children("table").children("tfoot").find("tr").each(function () {
                            $(this).show();
                        });
                        ActivityInstaneDiv.children("table").children("thead").find("td").each(function () {
                            $(this).show();
                        });
                        ActivityInstaneDiv.children("table").find("#CurrentActivityCode").html(_thisActivtyCode);
                        //设置尾部提示
                        //$("<div>单击流程节点或<a>点击这里</a>可以调整活动<div>").find("a").click(showAdjustActivityModal).append(ActivityInstaneDiv)
                ...
                if (typeof (ActivityInstaneDiv) != "undefined" && ActivityInstaneDiv.is(":visible")) {
                    ...
                    setTimeout(function () {
                        if (typeof (ActivityInstaneDiv) != "undefined" && ActivityInstaneDiv.is(":visible")) {
                            ...
                            if (InstaneTableLock && LeaveTime && LeaveTime != Number.POSITIVE_INFINITY && (new Date).getTime() - ActivityInstaneDiv.data("LeaveTime") > 900) {
                                ActivityInstaneDiv.hide();
                            }
                        }
                    }, 1000);
