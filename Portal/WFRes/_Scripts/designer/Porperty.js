            ...
            $(this).unbind("change.SelectAllDataItem").bind("change.SelectAllDataItem", function (e) {
                ...
                $(e.target).parents("table:first").find("tr").each(function (index) {
                    if (index != 0) {
                        ...
                        //通过disabled 判断是否可以取消勾选
                        if (_CheckBox.attr('disabled')!='disabled') {
        ...
        if (wp.Multi.IsMulti) {
            ...
            $(wp.Multi.CurrentObjects).each(function () {
                _filterProperties = $(WorkflowProperties[_PropertyType]).filter(function () {
                    return (HasSheet && this.Group == 'ActivityBasic') || this.Group == "Style" ||
                        (wp.Multi.isAllParticipative && (this.Group == "DataItem" || this.Group == "Operation" || this.Group == "ActivityEvent"));
                });
...
var DataPermissions = function (obj, parent) {
    ...
    if (!obj["DataPermissions"] || obj["DataPermissions"].length == 0) {
        ...
        $(_DataItems).each(function () { 
            //手工节点默认必填处理
            if (this.DefaultRequired && obj.ActivityType==3) {
    ...
    if (DataPermissionRows && DataPermissionRows.length > 0 && obj["DataPermissions"] && obj["DataPermissions"].length > 0) {
        $(obj["DataPermissions"]).each(function () {
            if (this.ItemName) {
                ...
                //显示属性
                _CurrentRow.find("input[type=checkbox]:eq(0)").prop("checked", this.Visible ? true : false).prop("disabled", this.IsDefault);
                _CurrentRow.find("input[type=checkbox]:eq(1)").prop("checked", this.Editable ? true : false).prop("disabled", this.IsDefault);
                _CurrentRow.find("input[type=checkbox]:eq(2)").prop("checked", this.Required ? true : false).prop("disabled", this.IsDefault);
                _CurrentRow.find("input[type=checkbox]:eq(3)").prop("checked", this.TrackVisible ? true : false);
                _CurrentRow.find("input[type=checkbox]:eq(4)").prop("checked", this.MobileVisible ? true : false).prop("disabled", this.IsDefault);
            }
        });
    }
            ...
            buttons: [
                ...
                },
               {
                   text: $.Lang("Designer.DataItem_Confirm"), onclick: function (item, dialog) {
                       ...
                       if (dialog.options.workflowProperty.Multi.IsMulti) {
                           wp.Multi.CurrentObjects.forEach(function (act, index) {
                               wp.ActivityEventEditor.ChangeActivity(act);
                               wp.ActivityEventEditor.SetActivity();
                           });
                       }
                       else {
