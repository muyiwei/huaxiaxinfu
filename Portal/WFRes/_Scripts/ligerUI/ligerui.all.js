    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        _getHtmlFromData: function (data, frozen) {
            ...
            for (var i = 0, l = data.length; i < l; i++) {
                var item = data[i];
                item['__index'] += parseInt((p.page - 1) * p.pageSize);
                var rowid = item['__id'];
                if (!item) continue;
                gridhtmlarr.push('<tr');
...
(function ($) {
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        getNodesByAttr: function (attrName, attrVal) {
            var g = this, p = this.options;
            if (!this.options.checkbox) return null;
            var nodes = [];
            $(".l-checkbox", g.tree).parent().parent("li").each(function () {
                var treedataindex = parseInt($(this).attr("treedataindex"));
                var data = g._getDataNodeByTreeDataIndex(g.data, treedataindex);
                if (data[attrName] == attrVal) {
                    nodes.push({ target: this, data: data })
                };
            });
            return nodes;
        },
