...
//创建表格
var CreateLigerGrid = function (divObj, gridColums, dataUrl, showCheckbox, height, width, tree, onAfterShowData, params, rownumbers) {
    var pageSize = 20;
    ...
    divObj.ligerGrid({
        ...
        onLoading: function (grid) {
            //处理全选按钮逻辑问题 加载数据去掉全选框选中状态
            var hrow = divObj.find(".l-grid-hd-row");
            hrow.removeClass("l-checked");
            return grid;
        },
        onAfterShowData: function () {
            if ($(".l-grid-row-cell-last").length > 0) {
                $(".l-grid-row-cell-last div").height("100%");
            }
            if (typeof (onAfterShowData) == 'function') {
                onAfterShowData();
            }
        }
    });
...
//对话框管理器
//ifMax :是否最大化
var ShowDialog = function (title, url, height, width, defaultTop) {
...
jQuery.extend(
    $.H3Dialog,
    {
        ...
        Success: function (option) {//添加提示成功！默认显示2s后自动关闭
            ...
            var maxHeight = 300;
            //计算文本宽度
            var preObject = $("<span>").addClass("h3-dialog-success").hide().html(option.content).appendTo(document.body);
            ...
            H3DialogObj.append('<span style="word-wrap:break-word;witdh:' + maxWidth + 'px;">' + option.content + '</span>');
            $("body").append(H3DialogObj.append(H3DialogObj));

            H3DialogObj.animate({ "top": ($(window).height() - objHeight) / 2 - 100 }, function () {
                var timer=setTimeout("RemoveDialog()", 5000);
        ...
        Warn: function (option) {//提示警告!标示可修复错误
            ...
            var maxHeight = 300;
            //计算文本宽度
            var preObject = $("<span>").addClass("h3-dialog-warn").hide().html(option.content).appendTo(document.body);
            var contentLength = preObject.width();
            var contentHeight = preObject.height();
            preObject.remove();
        ...
        Error: function (option) {//提示错误！标示不可修复的错误
            ...
            var maxHeight = 300;
            //计算文本宽度
            var preObject = $("<span>").addClass("h3-dialog-error").hide().html(option.content).appendTo(document.body);
            var contentLength = preObject.width();
            var contentHeight = preObject.height();
            preObject.remove();
        ...
        AlertMultiMsg: function (option) {
            ...
            for (var i = 0, j = option.content.length; i < j; i++) {
                ...
                rowObj.append('<span style="word-wrap:break-word;witdh:' + maxWidth + 'px;">' + option.content[i].text + '</span>');
